...
Ask2Know 是一个面向个人和小团队的低样本主动教学训练框架。v0.3.5 的重点不是做大模型，而是在进入 v0.4.0 前修稳：旧项目继续学习、训练集自动重命名、错因多选、提问上下文、自我总结和轻量数据增强。

## 安装

```bat
conda activate ask2know
cd D:\ask2know_framework\a2k_v0.3.5
...
如果系统把 grape 识别成 apple，用户纠正后，系统会追问为什么错。v0.3.5 支持多选：
...
v0.3.5 默认启用温和增强，用于建立类别原型：亮度、轻微旋转、轻微裁剪。可以在 `task_config.yaml` 中关闭：
