...
def add_new_object_if_needed(objects, label):
    ...
    objects.append({
        ...
        'name': label,
        'display_name': label,
        'description': 'added during v0.3.3 interactive learning'
    })
    return objects
...
def ask_correction_reason(predicted_label, true_label, pairwise_manager, adaptive_weights, sample_path=None):
    ...
    v0.3.5 supports multi-select answers because real differences often involve
...
def main():
    ...
    parser.add_argument('--no-preview', action='store_true', help='兼容旧参数；v0.3.5 默认就是不预览')
