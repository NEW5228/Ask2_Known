...
### 新增
- 用户纠正错误类别后，系统会追加一次“错误后追问”，询问主要错因。
- 新增 `metadata/pairwise_experience.json`，记录类别对混淆经验，例如 `apple vs grape`。
- 问题选择器开始参考类别对经验，优先提问历史上更有用的问题，降低低收益问题优先级。
- 经验报告会输出本轮错因追问记录。

### 保留
- 继续保留 v0.3.2 的持续学习、unlabeled 自动重命名、confirmed 自动进入长期训练库。

### 仍未完成
- 还没有深度特征、主体裁剪、数据增强。
- 类别对经验目前主要影响提问和权重，还不是强识别模型。
