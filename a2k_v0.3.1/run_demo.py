...
def add_new_object_if_needed(objects, label):
    ...
    objects.append({
        ...
        'name': label,
        'display_name': label,
        'description': 'added during v0.3.1 interactive learning'
    })
    return objects


def handle_sample_decision(decision, label, sample_path, model, pool, objects):
    if decision in ('confirmed', 'class', 'new') and label:
        objects = add_new_object_if_needed(objects, label)
        copied = pool.add_confirmed(sample_path, label)
        model.add_confirmed_sample(label, sample_path)
        print('已加入 confirmed 样本池:', label)
        print('保存副本:', copied)
        return {'decision': 'confirmed', 'label': label, 'copied_to': copied}
    if decision == 'candidate' and label:
        copied = pool.add_candidate(sample_path, label)
        print('已加入 candidate 样本池，不进入正式学习:', label)
        print('保存副本:', copied)
        return {'decision': 'candidate', 'label': label, 'copied_to': copied}
    if decision == 'reject':
        copied = pool.add_rejected(sample_path, 'rejected')
        print('已加入 rejected 样本池。')
        print('保存副本:', copied)
        return {'decision': 'rejected', 'label': None, 'copied_to': copied}
    print('已跳过，不进入样本池。')
    return {'decision': 'skip', 'label': None, 'copied_to': None}


def make_experience_report(cfg, logs, final_weights, question_state, objects):
    task = cfg.get('task', {})
    report = {
        'framework': 'Ask2Know',
        'version': VERSION,
...
def main():
    ...
    pool = SamplePoolManager(project_root=project_root, output_dir=output_dir)

    print_header(f'Ask2Know v{VERSION} - 低样本主动教学框架')
    ...
    for idx, sample in enumerate(unlabeled, 1):
        ...
        if global_uncertain:
            ...
            mode, label = ask_true_label(objects, allow_new=True, allow_reject=True)
            pool_info = handle_sample_decision(mode, label, sample_path, model, pool, objects)
            logs.append({
                'sample': sample_path,
                'before': results,
        ...
        if gap > ask_threshold:
            print('\n候选差距较大，系统暂不主动提问。')
            decision, label = confirm_prediction(results[0]['label'], objects)
            pool_info = handle_sample_decision(decision, label, sample_path, model, pool, objects)
            logs.append({
                'sample': sample_path,
                'before': results,
        ...
        decision, label = confirm_prediction(new_results[0]['label'], objects)
        pool_info = handle_sample_decision(decision, label, sample_path, model, pool, objects)
        helpful = False
        if pool_info.get('decision') == 'confirmed':
            ...
            helpful = new_gap >= old_gap and pool_info.get('label') == new_results[0]['label']
    ...
    question_state = q_selector.export()
    report = make_experience_report(cfg, logs, final_weights, question_state, objects)
