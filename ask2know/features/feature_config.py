...
def resolve_deep_feature_config(cfg):
    ...
    if not bool(merged.get('enable', True)):
        raise ValueError('Ask2Know v0.4.2.1n requires deep_features.enable: true with provider: open_clip.')
    provider = str(merged.get('provider', 'open_clip')).strip().lower()
    if provider not in ('clip', 'open_clip'):
        raise ValueError('Ask2Know v0.4.2.1n requires deep_features.provider: open_clip.')
