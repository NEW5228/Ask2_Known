...
def display_results(results, max_items=5):
    for i, r in enumerate(results[:max_items], 1):
        ...
        if r.get('subprototype_score') is not None:
            sources.append(f'subproto:{r["subprototype_score"]:.3f}')
        ...
        if r.get('hierarchy_score') is not None:
            sources.append(f'hier:{r["hierarchy_score"]:.3f}')
        if r.get('pairwise_score') is not None:
            sources.append(f'pairwise:{r["pairwise_score"]:.3f}')
        if r.get('crop_rerank_score') is not None:
            sources.append(f'crop:{r["crop_rerank_score"]:.3f}')
        if r.get('pair_confusion_score') is not None:
            sources.append(f'pair_conf:{r["pair_confusion_score"]:.3f}')
...
def ask_correction_reason(predicted_label, true_label, pairwise_manager, adaptive_weights, feature_spec, sample_path=None):
    ...
    v0.4.4 supports multi-select answers because real differences often involve
...
def main():
    ...
    parser.add_argument('--no-preview', action='store_true', help='兼容旧参数；v0.4.4 默认就是不预览')
    ...
    deep_cfg = deep_feature_config
    if deep_cfg.get('enable', False):
        print('Embedding 内部特征:', deep_cfg.get('provider', 'open_clip'), deep_cfg.get('feature_name', 'image_embedding'))
