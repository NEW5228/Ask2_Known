...
GENERIC_REASON_PROMPTS = {
    'color': '这两个类别是否主要因为颜色/色系差异而分开？',
    'shape': '这两个类别是否主要因为整体形状、结构、轮廓不同而分开？',
    'texture': '这两个类别是否主要因为表面纹理、颗粒感、重复结构不同而分开？',
    'cluster': '这张图是否更像多个小单元聚集，而不是单个主体？',
}
...
class PairwiseExperienceManager:
    def __init__(self, metadata_dir=None, path=None):
        if path is not None:
            self.path = Path(path)
        else:
    ...
    def _load(self):
        if self.path.exists():
            try:
                ...
                pass
        return {'schema_version': '0.3.4', 'pairs': {}}
    ...
    def get_feature_bias(self, a, b):
        ...
        useful = dict(pair.get('useful_features', {}))
        weak = dict(pair.get('weak_features', {}))
        return useful, weak
    ...
    def get_question_bias(self, a, b):
        ...
        for feature, count in useful.items():
            ...
            if qid:
                q_bonus[qid] = q_bonus.get(qid, 0.0) + min(float(count) * 0.15, 0.45)
    ...
    def record_correction(self, predicted, true_label, reason_id, reason_text=None, free_note=None):
        ...
        if reason_id in ('background', 'quality', 'other'):
            # Do not treat background/unknown as a useful class feature.
            pass
        elif reason_id == 'size':
    ...
    def suggest_pair_prompt(self, a, b):
        ...
        parts = []
        if useful:
            top_feature = useful[0][0]
            parts.append('历史经验里，这两个类别更适合参考：' + '、'.join([x[0] for x in useful[:3]]) + '。')
            if top_feature in hints:
                parts.append('更具体地说：' + hints[top_feature])
        if weak:
            parts.append('历史经验里，这些特征可能不可靠：' + '、'.join([x[0] for x in weak[:3]]) + '。')
        return ''.join(parts)

    def preferred_question_id(self, a, b):
        q_bonus, q_penalty = self.get_question_bias(a, b)
        if not q_bonus:
            return None
        return sorted(q_bonus.items(), key=lambda x: x[1], reverse=True)[0][0]

    def specific_question_text(self, a, b):
        pair = self.get_pair(a, b)
        useful = sorted(pair.get('useful_features', {}).items(), key=lambda x: x[1], reverse=True)
        hints = FRUIT_PAIR_HINTS.get(frozenset([a, b]), {})
        if useful:
            f = useful[0][0]
            if f in hints:
                return '根据历史错因经验，' + hints[f] + '请判断这条经验是否适用于当前图片。'
            if f in GENERIC_REASON_PROMPTS:
                return '根据历史错因经验，系统认为“' + f + '”可能有用。' + GENERIC_REASON_PROMPTS[f]
        return ''
    ...
    def correction_options(self, predicted, true_label):
        ...
        return [
            ...
            ('D', 'background', '背景、光线、遮挡或主体不清楚影响了判断。', {'increase': [], 'decrease': ['color', 'texture']}),
            ('E', 'other', '不确定 / 其他原因。', {'increase': [], 'decrease': []}),
        ]
