...
def _main_mask(img):
    # v0.3.4: use stable fallback by default. GrabCut caused Windows runs to appear stuck
    # on some real photos, especially when combined with image preview and file moving.
    # Future foreground modules can re-enable stronger segmentation behind an explicit option.
