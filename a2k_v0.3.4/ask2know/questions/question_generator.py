...
def generate_natural_question(top_a, top_b, question, weights, sample_path=None, pairwise_manager=None):
    ...
    pair_hint = ''
    specific_pair_question = ''
    if pairwise_manager is not None:
        pair_hint = pairwise_manager.suggest_pair_prompt(a, b)
        specific_pair_question = pairwise_manager.specific_question_text(a, b)
    ...
    if specific_pair_question:
        question_text = (intro + specific_pair_question)
    elif qid == 'Q_COLOR_IMPORTANCE':
        question_text = (
            intro
