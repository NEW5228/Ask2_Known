...
from ask2know.features.augmentation import generate_augmented_images
    ...
    def fit(self, samples):
        ...
        for sample in samples:
            ...
            if self.augmentation_config.get('enable', False):
                ...
                if img is not None:
                    for aug_img in generate_augmented_images(img, self.augmentation_config):
                        try:
                            feature_sets.append(extract_features_from_image(aug_img))
                        except Exception:
                            pass
            for one_feats in feature_sets:
                for name in self.feature_names:
                    if name in one_feats:
                        grouped[label].setdefault(name, []).append(one_feats[name])
        self.prototypes = {}
